...
def fetch_polyvore(data_path,
    ...
    def n_to_generate(generate_per_outfit, current_outfit):
        ...
        for _ in range(generate_per_outfit):
            ...
            new_outfit_score = round(1 - how_many_to_change / non_zero_items.shape[0], 4)
    ...
    def remap_data(input_data, polyvore_version='nondisjoint', limit_items=19, use_desc=False, generate_per_outfit=0):

        with open(data_path + "item_ids", "r") as fp:
            cols = json.load(fp)
        
        category_x_itemID = item_metadata[cols].transpose().reset_index().rename({'index': 'item_id'}, axis=1)
        ...
        for (line) in tqdm(lines, total=len(lines)):  
            ...
            outfit_dict = {
                'target': target, 
                'outfit_items': limit_n_pad_items(actual_items, limit_items).astype('U16'), 
                'item_categories': limit_n_pad_items(item_categories, limit_items), 
                'category_names': item_categories_names[:limit_items],
                'MID_target': np.zeros(limit_items) if target == 1 else np.ones(limit_items),
                'altered': False,
            ...
            count_nonzero = outfit_dict['outfit_items'][outfit_dict['outfit_items'] != '0'].shape[0]

            if target == 1 and generate_per_outfit > 0 and  count_nonzero > 2:
                
                for _ in range(generate_per_outfit):
                    
                    to_change_list, outfit_scores = n_to_generate(1, outfit_dict['outfit_items'])
                    ...
                    for i in range(len(to_change_list)):
                        ...
                        for x in to_change_list[0]:
                            positions_items_to_change.extend(np.where(outfit_dict['outfit_items'] == x)[0])
                        ...
                        for j in positions_items_to_change:
                            ...
                            substitute_item = choose_random_w_exception(candidates, current_item_id)

                            # category_id, item_text, vf
                            item_text = fetch_item_text(substitute_item, use_desc=use_desc)
    ...
    if use_misfits:
        to_save_path = data_path + 'misfits/' + polyvore_version + '_gen' + str(generate_per_outfit) + '_' + str(limit_items)

        if not os.path.isfile(to_save_path + '_train.pkl'):
            print("Generate outfits")
            
            train_df = remap_data(input_data='train', polyvore_version=polyvore_version, limit_items=limit_items, use_desc=use_descriptions, generate_per_outfit=generate_per_outfit)
            train_df = train_df.sample(frac=1).reset_index(drop=True)
            valid_df = remap_data(input_data='valid', polyvore_version=polyvore_version, limit_items=limit_items, use_desc=use_descriptions, generate_per_outfit=generate_per_outfit)
            test_df = remap_data(input_data='test', polyvore_version=polyvore_version, limit_items=limit_items, use_desc=use_descriptions, generate_per_outfit=generate_per_outfit)
            original_test_df = remap_data(input_data='test', polyvore_version=polyvore_version, limit_items=limit_items, use_desc=use_descriptions, generate_per_outfit=0)
        ...
        to_save_path = data_path + 'misfits/' + polyvore_version + '_default' + '_' + str(limit_items)
        
        if not os.path.isfile(to_save_path + '_train.pkl'):
            print("Generate outfits")

            print("Remap Polyvore")
            train_df = remap_data(input_data='train', 
            ...
            valid_df = remap_data(input_data='valid', 
                                  polyvore_version=polyvore_version, 
                                  limit_items=limit_items, 
                                  use_desc=use_descriptions, 
                                  generate_per_outfit=0)

            test_df = remap_data(input_data='test', 
                                 polyvore_version=polyvore_version, 
                                 limit_items=limit_items, 
                                 use_desc=use_descriptions, 
                                 generate_per_outfit=0)
...
def fetch_features(data_path, choose_image_model, pretraining_method):
    ...
    with open(data_path + 'features/' + choose_image_model + '_' + pretraining_method + "_item_ids", "r") as fp:
        cols = json.load(fp)

    vf = np.load(data_path + 'features/' + choose_image_model + '_' + pretraining_method + '_vf.npy')
    vf_df = pd.DataFrame(vf.T, columns=cols)      

    print('Load textual features')
    tf = np.load(data_path + 'features/' + choose_image_model + '_' + pretraining_method  + '_tf.npy')
