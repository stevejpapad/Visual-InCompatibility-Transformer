...
class FLIP(nn.Module):
    def __init__(
        ...
        self.image_encoder = torch.nn.Sequential(
            *(list(timm.create_model(self.choose_image_encoder, pretrained=True).children())[:-1])
        )
        ...
        if self.choose_text_encoder == "CLIP_Transformer":
            ...
            self.text_encoder = torch.nn.Sequential(
                *(list(freeze_model(AutoModel.from_pretrained(self.choose_text_encoder)).children())[:-1])
            )

        self.fcl_img = nn.Linear(self.img_models_params[self.choose_image_encoder]['emb_size'], 
                                 self.emb_dim)

        self.fcl_txt = nn.Linear(self.txt_models_params[self.choose_text_encoder]['emb_size'], 
                                 self.emb_dim)  
    ...
    def forward(self, images, texts):        
        ...
        if self.choose_image_encoder.startswith('vit') or self.choose_image_encoder.startswith('mixer'):
...
class DatasetIterator_ImageText(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.choose_text_model == "CLIP_Transformer":
            ...
            txt = np.array(self.tokenizer(current.text, padding="max_length", truncation=True)["input_ids"])        
...
def train_flip(choose_image_model, 
               EPOCHS = 20, 
               LEARNING_RATE = 1e-4, 
               emb_size = 512, 
               choose_gpu = 0, 
    ...
    def preprocess_text(text):
        punctuationfree="".join([i for i in text if i not in punctuation and not i.isdigit()])
    ...
    def remap_dataset_FLIP(input_data):
        category_x_itemID = item_metadata.transpose().reset_index().rename({'index': 'item_id'}, axis=1)
        category_x_itemID = category_x_itemID.groupby('category_id')['item_id'].apply(list)

        with open(data_path + polyvore_version + '/compatibility_'+ input_data +'.txt') as f:
            lines = f.readlines()
        ...
        for (line) in tqdm(lines, total=len(lines)):  
            ...
            outfit_idxs = [x.split('_')[0] for x in outfits_plus_idx] if target == 0 else [outfits_plus_idx[0].split('_')[0]]
            item_idxs = [x.split('_')[1] for x in outfits_plus_idx]

            count = 0
            actual_items, item_categories, item_categories_names, item_texts, visual_features = [], [], [], [], []
            ...
            if len(outfit_idxs) == 1:
                ...
                for i in range(len(item_idxs)):        
                    current_item = current_outfit_df[i][int(item_idxs[i]) - 1]['item_id']
    ...
    if not os.path.isfile(data_path + 'misfits/' + 'FLIP_' + polyvore_version +'_train.pkl'):
        ...
        train_df.to_pickle(data_path + 'misfits/' + 'FLIP_' + polyvore_version + '_train.pkl')
        valid_df.to_pickle(data_path + 'misfits/' + 'FLIP_' + polyvore_version + '_valid.pkl')
        test_df.to_pickle(data_path + 'misfits/' + 'FLIP_' + polyvore_version + '_test.pkl')

    else:    
        print("Load re-mapped Polyvore")
        train_df = pd.read_pickle(data_path + 'misfits/' + 'FLIP_' + polyvore_version + '_train.pkl')
        valid_df = pd.read_pickle(data_path + 'misfits/' + 'FLIP_' + polyvore_version +  '_valid.pkl')
        test_df = pd.read_pickle(data_path + 'misfits/' + 'FLIP_' + polyvore_version + '_test.pkl')
    ...
    model = FLIP(img_models_params=img_models_params,
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=LEARNING_RATE, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.001)
    ...
    for epoch in range(EPOCHS):
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            print(f"[Epoch:{epoch + 1}, Batch:{i + 1:5d}/{batches_per_epoch}]. Passed time: {round((time.time() - epoch_start_time) / 60, 1)} minutes. loss: {running_loss / (i+1):.3f}",
                  end="\r")
...
def extract_visual_features(data_path, 
                            choose_image_model, 
                            choose_text_model, 
                            pretraining_method, 
                            batch_size = 64,
    ...
    print("Load re-mapped Polyvore")
    train_df = pd.read_pickle(data_path + 'misfits/' + 'FLIP_' + 'nondisjoint' + '_train.pkl')
    valid_df = pd.read_pickle(data_path + 'misfits/' + 'FLIP_' + 'nondisjoint' + '_valid.pkl')
    test_df = pd.read_pickle(data_path + 'misfits/' + 'FLIP_' + 'nondisjoint' + '_test.pkl')
    ...
    dataloader = DataLoader(all_data_dg, 
                            batch_size=batch_size, 
                            shuffle=False, 
                            num_workers=8, 
                            pin_memory=True)
    ...
    flip_model = FLIP(img_models_params=img_models_params,
                      txt_models_params=txt_models_params,
    ...
    np.save(data_path + 'features/' + choose_image_model + '_' + pretraining_method + '_vf.npy', all_vf)
    np.save(data_path + 'features/' +choose_image_model + '_' + pretraining_method + '_tf.npy', all_tf)

    with open(data_path + 'features/' + choose_image_model + '_' + pretraining_method + "_item_ids", "w") as fp:
